...
SC_CubeBinding.prototype = {
  ...
  , resolve : function(){
	   // if(this.name == 'evtChange') console.log('resolve', this.cube)
      if(null === this.cube){
 ...
 */
  , wakeupAll : function(m, flag){
      for(var idx = 0; idx < this.registeredInst.length; idx++){
			// console.log('===========')
			// console.log(this.registeredInst[idx])
			// console.log(SC_Opcodes.toString(this.registeredInst[idx].oc))
			// // console.log(SC_Opcodes.toString(60))
			// console.log('- =========== -')
 ...
 */
  ...
  , generateValues : function(m, val){
		// console.log('generateValues', val)
      if(undefined !== val){
        this.vals.push(val);
        }
      }
  ...
  , getValues : function(m){
		// console.log('getValues', this.vals)
      if(!this.isPresent(m)){
  ...
  , getAllValues : function(m, vals){
      vals[this] = this.getValues(m);
		// console.log('getAllValues', vals[this])
      }
      ...
      switch(this.oc){
        ...
        case SC_Opcodes.AWAIT:{
          ///console.log("await" , SC_Opcodes.toString(this.oc));
			 // console.log('SC_Opcodes.AWAIT', this.config)
          if(this.config.isPresent(m)){
				// console.log('SC_Opcodes.AWAIT int', this.config.name)
            return SC_Instruction_State.TERM;
            }
        ...
        case SC_Opcodes.AWAIT_REGISTRED:{
          //console.log("await registered " , SC_Opcodes.toString(this.oc));
			 // console.log('SC_Opcodes.AWAIT_REGISTRED', this.config)
          if(this.config.isPresent(m)){
            this.reset(m);
            return SC_Instruction_State.TERM;
            }
      ...
      switch(this.oc){
        ...
        case SC_Opcodes.ACTION_ON_EVENT_NO_DEFAULT_REGISTERED:{
          if(this.evtFun.config.isPresent(m)){
				 ...
				 // console.log(SC_Opcodes.toString(this.path.oc))
            res = this.path.awake(m, flag);
            }
          return false;
          }
...
SC_Seq.prototype = {
  ...
  , bindTo : function(engine, parbranch, seq, masterSeq, path, cube){
      ...
      for(var i = 0; i < this.seqElements.length; i++){
			// console.log('num', i)
			// console.log('elt', copy.seqElements[i])
      ...
      if(p instanceof SC_Par){
        ...
        for(var n = 0 ; n < p.branches.length; n ++){
          this.addBranch(p.branches[n].prg, pb, engine);
			...
			// console.log('debut addBranch else')
        var b = new SC_ParBranch(pb, this, SC_Nothing);
			// console.log('milieu addBranch else', p, this.cube)
        ...
        if(b.hasPotential){
          ...
          if(b.idxInProd<0){
			...
			// console.log('fin addBranch else')
        }
      }
...
SC_ParDyn.prototype.bindTo = function(engine, parbranch, seq, masterSeq, path, cube){
  ...
  copy.cube = cube;
  var tmp = this.suspended.start;
  // console.log('prg 1', tmp.prg)
  // console.log('prg 2', tmp.prg.suspended.start.prg)
  while(null != tmp){
    var b = new SC_ParBranch(parbranch, copy, SC_Nothing);
    b.prg = tmp.prg.bindTo(engine, b, null, masterSeq, b, cube);
    b.path = copy;
...
function SC_Machine(delay, params){
  ...
  // this.systemEvent = function(target, name){
    ...
    // }
  // }
  this.systemEvent = (target, name)=>{
      // var SC_event = new SC_Sensor(""+target+name, true);
      var SC_event = new SC_Event(""+target+name);
		// console.log('sysevt', name)
      target.addEventListener(name, evt=>{
			evt.preventDefault()
			console.log('evtlisten1', name)
         this.generateEvent(SC_event, evt)
			console.log('evtlisten2', name)
		});
      return SC_event;
  }
}
SC_Machine.prototype = 
{
  ...
  , react : function(){
		// if(this.instantNumber > 4) return;
		// console.log('debut react ' + this.instantNumber)
      var res = SC_Instruction_State.STOP;
      var tmp = this.pending;
		// console.log('pending', tmp)
      this.pending=[];
      for(var n in tmp){
      ...
      for(var i in tmp){
			// console.log(i)
			// console.log(tmp[i])
		  // console.log('inter react ' + this.instantNumber, i)
      ...
      if((SC_Instruction_State.OEOI == res)||(SC_Instruction_State.WEOI == res)){
		...
		// console.log(this.instantNumber, "<--- phase2 ---");
      ...
      if(this.traceEvt.isPresent(this)){
		...
		// console.log('fin react ' + this.instantNumber)
      this.instantNumber++;
		// console.log('vrai debut react ' + this.instantNumber)
...
SC_Cube.prototype = {
  ...
  , bindTo : function(engine, parbranch, seq, masterSeq, path, cube){
      // console.log("debut adding on bind (cube)");
      ...
      // console.log("milieu adding on bind (cube)");
      ...
      copy.m = engine;
      copy.pb = parbranch;
      // console.log("fin adding on bind (cube)");
      return copy;
      }
  , toString : function(){
...
SC = {
  ...
  log: function(msg){
    ...
    //console.log("preparing log for : ",msg)
	 const instr = SC.action(act.bind(null, msg));
	 instr.infoSuppl = 'log_' + msg
    return instr;
